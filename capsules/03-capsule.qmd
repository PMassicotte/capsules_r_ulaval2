...
{{< video https://youtu.be/KBkVAx2YW9E >}}
...
check_question(
  ...
  right = "✅ Correct !",
  wrong = "❌ Incorrect. Mais vous êtes proche! Les fonctions ont rarement un nom en français.",
  title = "Quelle fonction devriez-vous utiliser pour calculer la moyenne des valeurs du vecteur suivant `x <- c(1, 2, 3)`"
)
```
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct !  `x` est un paramètre obligatoire de la fonction `mean()` puisque que ce paramètre ne possède pas de valeur par défaut.",
  wrong = "❌ Incorrect. Le paramètre `trim` à une valeur par défaut égale à `0`. Il n'est pas obligatoire de lui spécifier une valeur. Le paramètre `na.rm` à une valeur par défaut égale à `FALSE`. Il n'est pas obligatoire de lui spécifier une valeur.",
  title = "Indiquez les paramètres que l'utilisateur doit obligatoirement fournir à la fonction."
)
```
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct !  La valeur par défaut est `FALSE` et il serait possible de la changer pour la valeur `TRUE` si l'on souhaite supprimer les valeurs manquantes avant de faire le calcul de la moyenne.",
  wrong = "❌ Incorrect.  Utilisez l'aide pour vérifier quelle est la valeur par défaut du paramètre `na.rm`.",
  title = "Valeur par défaut de `na.rm`"
)
```
...
check_question(
  c(
    "rnorm(100, 50, 2)",
    "rnorm(sd = 2, mean = 50, 100)",
    "rnorm(sd = 2, mean = 50, n = 100)",
  ...
  options = c(
    ...
    "rnorm(sd = 2, mean = 50, 100)",
    "rnorm(sd = 2, mean = 50, n = 100)",
    "rnorm(x = 100, sd = 2, mean = 50)",
    "rnorm(100, mean = 50, sd = 2)"
  ),
  type = "checkbox",
  button_label = "Vérifier",
  right = "✅ Correct !  `rnorm(100, 50, 2)` est équivalent à `rnorm(n = 100, mean = 50, sd = 2)`.",
  wrong = "**Incorrect** Le paramètre `x` ne fait pas partie de la liste des paramètres utilisables avec la fonction `rnorm()`.",
  title = "Parmi les choix suivants, lesquels permettent techniquement de générer une séquence de 100 nombres ayant un écart-type de 2 et une moyenne de 50?"
)
```
...
2. Elle accepte des paramètres (aussi appelés _arguments_, surtout dans la documentation en anglais) entre les parenthèses qui suivent son nom.
...
[^1]: Voir la capsule #2 _Les variables dans R_ pour bien comprendre la notion de vecteur de données.
...
1. _Paramètres obligatoires_: L'utilisateur doit nécessairement fournir une valeur.
2. _Paramètres optionnels_: Une valeur par défaut est déjà spécifiée. Dans l'aide de la fonction, les paramètres optionnels sont précédés par le signe "=".

...
check_question(
  ...
  right = "✅ Correct ! Les valeurs de 'banane' et de '3.1415' sont entourées de guillemets ce qui indique des valeurs de type caractère.",
  wrong = "❌ Incorrect.  N'oubliez pas que les caractères sont entourés de guillemets (simples ou doubles).",
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct ! L'opérateur ` <- ` est recommandé en plus d'éviter la confusion avec l'opérateur logique `==`.",
  wrong = "❌ Incorrect.  L'opérateur `=` fonctionne, mais il est recommandé d'utiliser ` <- ` pour éviter la confusion avec l'opérateur logique `==`.",
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct ! `class()` est une fonction de base de R pour déterminer la classe d'une variable.",
  wrong = "❌ Incorrect.  Essayez encore! Utilisez les indices si nécessaire au bas de la question.",
...
check_question(
  ...
  type = "checkbox",
  button_label = "Vérifier",
  right = "✅ Correct ! Puisque les noms de mois sont fixes, le type `factor` est le meilleur choix. Le type `character` est également valide.",
  wrong = "❌ Incorrect. Il y a peut-être plus d'une bonne réponse!",
...
check_question(
  ...
  type = "checkbox",
  button_label = "Vérifier",
  right = "✅ Correct ! `x2` est valide puisqu'il commence par une lettre, ne contient pas de caractères spéciaux et/ou d'espaces. `température` est techniquement valide, mais il est fortement conseillé d'éviter l'utilisation de caractères spéciaux.",
  wrong = "❌ Incorrect. Un nom de variable ne doit pas commencer par une valeur numérique ou contenir d'espace.",
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct ! Ce nom est descriptif en plus d'indiquer les unités.",
  wrong = "❌ Incorrect. Le nom de la variable (`x`) n'est pas très descriptif ou les unités ne sont pas indiquées.",
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct ! `c()` est la fonction pour créer un vecteur en plus de ne pas être un nom de variable très descriptif.",
  wrong = "❌ Incorrect. Attention, il y a peut-être plusieurs réponses vraies!",
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct ! Puisqu'il y a une valeur texte dans le vecteur,`2`, le vecteur entier est de type `charactère`.",
  wrong = "❌ Incorrect. Utilisez la fonction `class()` pour valider votre choix.",
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct !  R n'utilise généralement pas des noms français.",
  wrong = "❌ Incorrect. Stictement parlant, `c(0, 1)` n'est pas `logic`. Essayez les commandes suivantes dans R: (1) `1 == TRUE` et (2) `0 == FALSE`.",
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct ! 42 est la valeur à la position 2 du vecteur `x`.",
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct ! `x[-2]` supprime la valeur à la position 2 du vecteur `x`.",

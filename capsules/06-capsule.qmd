...
Avec la fonction `table()`, il est également possible de faire un décompte croisé, c'est à dire selon plusieurs variables à la fois. Dans l'exemple suivant, nous faisons un décompte du nombre d'observations _Face_ et _Pile_ en groupant par `genre` et `piece_monnaie`. On peut ainsi constater immédiatement que les nombres de résultats _Face_ et _Pile_ sont presque égaux (comme on peut s'y attendre) aussi bien pour les `Filles` que les `Gars`.

...
check_question(
  ...
  right = "✅ Correct ! La fonction hist() permet de faire des histogrammes.",
  wrong = "❌ Incorrect. Consulter le matériel accompagnateur ou la capsule vidéo pour connaître la bonne fonction à utiliser.",
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct ! Il faut que la variable soit de type numérique (continue).",
  wrong = "❌ Incorrect. Rappelez-vous qu'un histogramme est une représentation graphique permettant de représenter la répartition d'une variable continue.",
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct ! Le paramètre breaks permet de contrôler le nombre *bins*.",
  wrong = "❌ Incorrect. Consulter l'aide de la fonction avec la commande ?hist.",
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct ! La fonction générique plot() permet de faire des graphiques nuage de points.",
  wrong = "❌ Incorrect. Consulter le matériel accompagnateur ou la capsule vidéo pour connaître la bonne fonction à utiliser.",
  title = "Quelle fonction permet de faire des graphiques nuage de points (*scatterplot*)?"
)
```
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct ! La fonction barplot() permet de faire des graphiques à bâtons.",
  wrong = "❌ Incorrect. Consulter le matériel accompagnateur ou la capsule vidéo pour connaître la bonne fonction à utiliser.",
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct ! La fonction table() utilise les facteurs de classification croisée pour créer un tableau de contingence des comptes à chaque combinaison de niveaux de facteurs.",
  wrong = "❌ Incorrect. classify() et count() n'existent à la base dans R. sort() permet de trier un vecteur numérique. Consulter le matériel accompagnateur ou la capsule vidéo pour connaître la bonne fonction à utiliser.",
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct ! La fonction boxplot() permet de faire des graphiques à moustaches.",
  wrong = "❌ Incorrect. La fonction moust() n'existe à la base dans R. La fonction box() permet d'ajouter une bordure autours d'un graphique. Consulter le matériel accompagnateur ou la capsule vidéo pour connaître la bonne fonction à utiliser.",
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct ! Le *tilde* se lit *en fonction de*.",
  wrong = "❌ Incorrect. + est l'opérateur d'addition. | est l'opétateur logique de comparaison *ou*. / est l'opérateur de division. ",
...
check_question(
  ...
  type = "radio",
  button_label = "Vérifier",
  right = "✅ Correct ! Le format vectoriel a une meilleure qualité comparée au format matriciel et il peut s’agrandir indéfiniment sans perte de résolution.",
  wrong = "❌ Incorrect. Ceux-ci sont plutôt des formats de fichiers de type matriciel.",
...
check_question(
  ...
  button_label = "Vérifier",
  right = "✅ Correct ! ",
  wrong = "❌ Incorrect. Il faut d'abord ouvrir le moteur graphique.",
...
check_question(
  ...
  button_label = "Vérifier",
  right = "✅ Correct ! ",
  wrong = "❌ Incorrect. Dans certains cas, le format vectoriel peut produire des fichiers volumineux. Le format matriciel peut aussi être lu sur n’importe quelle plateforme (Windows, Mac, Linux). ",
...
| pairs(df)  | Permets de visualiser la relation entre toutes les variables continues d’une structure de données (tableau de données _data frame_). |
...
Nous utiliserons un jeu de données biologiques intéressant afin de présenter les fonctions de base de chacun de ces types de graphiques. En effet, R contient des jeux de données qui ont été créés et intégrés à la version de base afin de nous permettre de tester ses différentes fonctions. Nous utiliserons le jeu de données _iris_ qui contient de véritables mesures morphologiques sur trois espèces d'Iris (des fleurs), récoltées par le biologiste Edgar Anderson. Ce jeu de données est d'autant plus intéressant pour nous que deux des trois espèces étudiées proviennent de la Gaspésie (incluant l'emblème floral du Québec, l'_Iris versicolor_). Ce jeu de données est aussi très connu, car il a été utilisé par Ronald Fisher, un des pères fondateurs de la biostatistique, dans un de ses articles scientifiques.
...
Nous apprenons que nous avons 150 observations dans notre tableau (_data frame_). Les données sur les sépales et les pétales sont numériques (_numeric_) alors que la variable décrivant les espèces est un facteur (_factor_).
...
[^1]: Voir la capsule intitulée _Les variables dans R_.
...
::: {.callout-note}
La librairie languageR n'est pas installée dans la version de base de R. Vous devrez l'installer vous-même[^3].
:::
...
Avec la fonction table(), il est également possible de faire un décompte croisé, c'est à dire selon plusieurs variables à la fois. Dans l'exemple suivant, nous faisons un décompte du nombre d'observations _Face_ et _Pile_ en groupant par genre et piece*monnaie. On peut ainsi constater immédiatement que les nombres de résultats \_Face* et _Pile_ sont presque égaux (comme on peut s'y attendre) aussi bien pour les Filles que les Gars.
...
1. Le format vectoriel (_eps_, _pdf_, _svg_, _ps_, etc).
2. Le format matriciel (_bmp_, _jpeg_, _png_, _tif_, etc.)

La plupart des journaux scientifiques préfèrent le format vectoriel lors du processus de soumission d'un article, car ce format de graphique a une meilleure qualité comparée au format matriciel et il peut s'agrandir indéfiniment **sans perte de résolution** (qualité). Les deux figures suivantes permettent de comparer la qualité graphique des formats _jpeg_ (format matriciel) et _pdf_ (format vectoriel). On voit qu'il y a de la pixellisation dans la version matricielle tandis que la version vectorielle est nettement plus claire. Si vous devez néanmoins utiliser le format matriciel, on vous recommande d'utiliser le format _png_ qui offre un bon compromis entre la qualité de l'image et la taille de fichier.
...
1. Ouvrir le moteur graphique lié au format de fichier que l'on veut (_pdf_, _png_, _jpeg_, etc.).

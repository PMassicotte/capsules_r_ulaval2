...
check_question(
  ...
  wrong = "**Attention!** Plusieurs choix peuvent être possibles.",
...
check_question(
  ...
  wrong = "**Mauvaise réponse** `sep` permet de spécifier le caractère utilisé pour séparer les colonnes de variables. `header` permet de spécifier si la première ligne du fichier correspond aux noms des variables (colonnes).",
...
#FIX: This should be a paged table by default as defined in the front matter. I
# do not know why this is not working. I am forcing it here until I find a
# solution.
rmarkdown::paged_table(sondage_bio1006)
```
...
```{webr}
#| autorun: false
sondage_bio1006 <- read.table("https://tinyurl.com/bio1006csv")
head()
```
...
check_question(
  ...
  wrong = "**Mauvaise réponse** L'indexation des *data frame* fonctionnent de la même manière que pour les matrices `M[ligne][colonne]`.",
